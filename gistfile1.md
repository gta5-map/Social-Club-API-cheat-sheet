...
__Description__: Display general informations about your (or others, if you fill the _nickname_ parameter) online character: 
...
__Response format__: HTML

#### Statistics

__Description__: More detailed statistics of general stuff such as: 
...
__Description__:  Kill death ratio, heatshots, shots fired, drive-by kills and overall accuracy. For detailed informations per weapon checkout the detailed information requests below.
...
__Response format__: HTML (and JSON hidden in `<script>`-tag)

##### Per weapon statistics

__Description__:  Returns detailed information for the given weapon such as:
...
__Description__:  Shows stats (name, primary color, secondary color, etc.) of the vehicles in your main (?) garage. Note: The info is hidden in a `<script></script>`-tag that stored the informations as JSON. If you need more informations you can also use the per-car requests.
...
__Description__:  This will also show additional the specs of the car (acceleration, handling, braking, etc.) as well as the car lights (Xeon/Non-xeon), suspension and more. Make sure to replace "[0-9]" with a number between 0 and 9. At the moment I don't know how to access cars from the different garages.
...
__Description__:  General informations about driven vehicles ingame: fastest speed, amount of stolen vehicles, farthest jump, and more.
...
__Description__:  This gives you general informations about all the vehicles in the GTA world. Speed, acceleration, amount of seats, if it's a moddable vehicle, if it's storable or sellable, etc.
...
__Description__:  Gives you information about the earned awards and it's completion statuses.
...
__Description__:  Returns informations about several minigames. Unfortunately you can't access the stats of other players, so all the requests below return your own (the account who authorized the requests) statistics.
...
__Description__:  Show stats about shooting range challenges. Make sure to replace __[CHALLENGE_TYPE]__ with one of the available types: 
